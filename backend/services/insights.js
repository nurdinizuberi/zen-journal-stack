// backend/services/insights.js — rule-based cross-module pattern detection (mirror of frontend lib/insights.ts)
// "Gentle suggestions, never diagnoses."

const DAY_MS = 24 * 60 * 60 * 1000;
const WEEKDAY_LABELS = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
const MOOD_EMOJI = {
  Calm: '🌿', Focused: '🎯', Grateful: '🙏', Courageous: '🦁', Energized: '⚡',
  Patient: '🕊️', Anxious: '🌊', Reflective: '🌙', Tired: '🌧️', Peaceful: '🏞️',
};
const HOUR_BUCKETS = [
...
export function computeAdvanced(entries, todos, goals, books, intentions) {
  const now = new Date();
  const words = (content) => {
  ...
  const completedTaskDays = new Set();
  for (const t of todos || []) {
    if (t.isCompleted) completedTaskDays.add(isoDayKey(t.updatedAt || t.createdAt));
  }

  const avgWordsFor = (groups) => {
  ...
  const observations = [];
  const push = (o) => observations.push(o);
  ...
  for (const key of dayKeys) {
    ...
    if (prev && date.getTime() - prev.getTime() === DAY_MS) run += 1;
    else run = 1;
    longestStreak = Math.max(longestStreak, run);
    prev = date;
  }
  if (longestStreak >= 5) {
    push({ icon: '🔥', title: 'Your longest stride', message: `You showed up ${longestStreak} days in a row — the longest stretch you've kept so far.`, tone: 'positive' });
  }
  ...
  const moodDeltas = {};
  for (const m of new Set([...Object.keys(cur), ...Object.keys(prevMoods)])) {
    moodDeltas[m] = (cur[m] || 0) - (prevMoods[m] || 0);
  }
  const rising = Object.entries(moodDeltas).sort((a, b) => b[1] - a[1])[0];
  if (rising && rising[1] >= 2 && (cur[rising[0]] || 0) >= 3) {
    push({ icon: moodEmoji(rising[0]), title: `${rising[0]} is rising`, message: `You've logged ${rising[0].toLowerCase()} ${cur[rising[0]]}× this week, up from ${prevMoods[rising[0]] || 0} last week.`, tone: 'positive' });
  }
  const falling = Object.entries(moodDeltas).filter(([, delta]) => delta <= -2).sort((a, b) => a[1] - b[1])[0];
  if (falling && (prevMoods[falling[0]] || 0) >= 3) {
    push({ icon: '🌒', title: `${falling[0]} shows up less`, message: `You've recorded ${falling[0].toLowerCase()} ${prevMoods[falling[0]]}× last week but only ${cur[falling[0]] || 0}× this week.`, tone: 'neutral' });
  }
  ...
  if (intentionDaysTotal >= 3 && withWords >= 1 && withoutWords >= 1 && withWords >= withoutWords + 12) {
    push({ icon: '💡', title: 'Your intention sets the tone', message: `Reflections written on days you set an intention run ${withWords} words on average vs ${withoutWords} on other days — roughly ${Math.max(15, Math.round(((withWords - withoutWords) / withoutWords) * 100))}% deeper.`, tone: 'positive' });
  } else if (intentionDaysTotal >= 3 && withMood && withoutMood && withMood !== withoutMood) {
    push({ icon: '🧭', title: 'Morning intention shifts the mood', message: `${withMood} shows up most on intention days, while ${withoutMood} colors the others. A quiet habit with a visible effect.`, tone: 'positive' });
  }

  // tasks → reflection depth
  ...
  if (taskDays.length >= 3 && taskDaysAvg >= 1 && noTaskDaysAvg >= 1 && taskDaysAvg >= noTaskDaysAvg + 12) {
    push({ icon: '☑', title: 'Action deepens reflection', message: `On days you completed at least one task, your writing ran ${taskDaysAvg} words vs ${noTaskDaysAvg} on quieter days.`, tone: 'positive' });
  }
  ...
  if (entries.length >= 5 && topDayCount >= 3 && topDayCount / entries.length >= 0.22) {
    push({ icon: '📅', title: `Your most reflective day: ${WEEKDAY_LABELS[topDayIdx]}`, message: `${Math.round((topDayCount / entries.length) * 100)}% of your reflections land on ${WEEKDAY_LABELS[topDayIdx]}.`, tone: 'neutral' });
  }
  ...
  const hourBuckets = HOUR_BUCKETS.map((b) => ({ label: b.label, count: hourCounts[b.label] || 0 }));
  ...
  if (entries.length >= 6 && topBucket && topBucket[1] / entries.length >= 0.45) {
    push({ icon: '⏰', title: `A natural ${topBucket[0].toLowerCase()} writer`, message: `${Math.round((topBucket[1] / entries.length) * 100)}% of your reflections were written ${topBucket[0] === 'Late night' ? 'late at night' : `in the ${topBucket[0].toLowerCase()}`}.`, tone: 'neutral' });
  }
  ...
  for (const e of entries) {
    ...
    for (const tag of Array.isArray(e.tags) ? e.tags : []) keys.add(String(tag).replace(/^#/, ''));
    for (const key of keys) {
      if (!themeEntries[key]) themeEntries[key] = [];
      themeEntries[key].push(e);
    }
  }
  const dominantMoodOf = (list) => {
    const counts = {};
  ...
  const themeMoods = Object.entries(themeEntries)
  ...
  if (topThemeMood && topThemeMood.moodShare >= 40) {
    push({ icon: moodEmoji(topThemeMood.mood) || '🏷️', title: `${topThemeMood.theme} and ${topThemeMood.mood.toLowerCase()} travel together`, message: `${topThemeMood.mood} appears in ${topThemeMood.moodShare}% of your ${topThemeMood.theme.toLowerCase()} reflections (${topThemeMood.count} total).`, tone: 'neutral' });
  }
  ...
  if (goalReflections.length > 0) {
    ...
    push({ icon: '◎', title: `${topGoal.title} keeps coming up`, message: `You've reflected on "${topGoal.title}" ${topGoal.reflections} times — that goal clearly matters to you right now.`, tone: 'positive' });
  }
  ...
  if (bookReflections.length > 0) {
    ...
    push({ icon: '📖', title: `${topBook.title} is becoming real learning`, message: `You've linked ${topBook.reflections} reflections to "${topBook.title}" — reading is turning into thinking.`, tone: 'positive' });
  }
  ...
  const dominantLifeArea = topArea && topArea[1] / Math.max(1, entries.length) >= 0.6 ? topArea[0] : null;
  if (dominantLifeArea && entries.length >= 6) {
    push({ icon: '🧿', title: `Your focus orbits ${dominantLifeArea.toLowerCase()}`, message: `${Math.round((topArea[1] / entries.length) * 100)}% of your reflections live in ${dominantLifeArea}.`, tone: 'neutral' });
  }

  // gentle early nudge
  if (entries.length === 0) {
    push({ icon: '🌱', title: 'Everything begins with one reflection', message: 'Your patterns will start to appear after a few reflections. Write the first one and come back — insights grow quietly.', tone: 'gentle' });
  } else if (entries.length < 4) {
    push({ icon: '🌱', title: 'You are at the start', message: 'A handful of reflections is where the story begins. Keep showing up and the patterns will surface on their own.', tone: 'gentle' });
  }

  // cap at seven, meaningful first
  const priorityRank = { positive: 0, neutral: 1, gentle: 2 };
  const observationsFinal = observations.sort((a, b) => priorityRank[a.tone] - priorityRank[b.tone]).slice(0, 7);

  const dayOfWeek = WEEKDAY_LABELS.map((label, idx) => ({ label, count: weekdayCounts[idx] }));
